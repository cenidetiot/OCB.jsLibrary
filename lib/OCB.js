...
class OCB { 
    ...
    testConnect(headers = {}){
        ...
        const promise = new Promise(function (resolve, reject) {
            ...
            .then((res) => {
                if(res.status >= 200 && res.status <= 208){
                    resolve({statusCode : res.status, message: "Connection successful with the Orion ContextBroker"})
                }
                else if (res.status === 404){
    ...
    createEntity(entity, headers = {}){
        ...
        const promise = new Promise(function (resolve, reject) {
            ...
            fetch(t._URLContext+'/entities', options)
                .then(function(res) {                    
                    if(res.status >= 200 && res.status <= 208){
                        resolve({statusCode : res.status, message:"Entity created successfully"})
                    }
    ...
    updateJSONAttrEntity(idEntity, nameAttribute, jsonAttr, headers = {}){
        ...
        const promise = new Promise(function (resolve, reject) {
            ...
            fetch(t._URLContext+'/entities/'+idEntity+'/attrs/'+nameAttribute, options)
                .then(function(res) {             
                    if(res.status >= 200 && res.status <= 208){
                        resolve({statusCode : res.status, message: "The attribute JSON '"+nameAttribute+"'"+" has been updated successfully in the entity: '"+idEntity+"'"})
                    }
                    else if (res.status === 404){
    ...
    replaceAllEntityAttributes(idEntity, jsonObjectAttrs, headers = {}){
        ...
        const promise = new Promise(function (resolve, reject) {
            ...
            fetch(t._URLContext+'/entities/'+idEntity+'/attrs/', options)
                .then(function(res) {             
                    if(res.status >= 200 && res.status <= 208){
                        resolve({statusCode : res.status, message: "The attributes of the entity with id: "+idEntity+ " has been replaced successfully"})
                    }
                    else if (res.status === 404){
                       reject(new Error("Request failed. Entity not found"))
                    }
            ...
            fetch(t._URLContext+'/subscriptions', options)
                .then(function(res) {                    
                    if(res.status >= 200 && res.status <= 208){
                        ...
                        resolve({statusCode : res.status, message: "Subscription created successfully"})
                    }
                    else if (res.status === 422){
    ...
    updateSubscription(idSubscription, jsonSubscription, headers = {}){
        ...
        const promise = new Promise(function (resolve, reject) {
            ...
            fetch(t._URLContext+'/subscriptions/'+idSubscription, options)
                .then(function(res) {             
                    if(res.status >= 200 && res.status <= 208){
                        resolve({statusCode : res.status, message: "The subscription: "+idSubscription+" has been updated successfully"})
                    }
                    if (res.status === 404){
                       reject(new Error("Request failed. Subscription not found"))
                    }
